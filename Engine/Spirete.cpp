...
//初期化
HRESULT Spirete::Load(LPCWSTR fileName)
{
	//頂点情報
...
//描画
void Spirete::Draw(Transform transform,int WINDOW_WIDTH,int WINDOW_HEIGHT)
{
	Direct3D::SetShader(SHADER_2D);
	transform.Calclation();
	//コンスタントバッファに情報を渡す
	PassDataToCB(transform.GetWorldMatrix(), WINDOW_WIDTH, WINDOW_HEIGHT);
...
//頂点バッファを作成
HRESULT Spirete::CreateVertexBuffer()
{
	HRESULT hr;
...
//インデックスバッファを作成
HRESULT Spirete::CreateIndexBuffer()
{
...
//コンスタントバッファ作成
HRESULT Spirete::CreateConstantBuffer()
{
...
//テクスチャをロード
HRESULT Spirete::LoadTexture(LPCWSTR fileName)
{
	pTexture_ = new Texture;
	HRESULT hr;
	hr = pTexture_->Load(fileName);
	WIDTH = pTexture_->GetWidth();
	HEIGHT = pTexture_->GetHeight();
	if (FAILED(hr))
	{
...
//コンスタントバッファに各種情報を渡す
void Spirete::PassDataToCB(DirectX::XMMATRIX worldMatrix,int WINDOW_WIDTH ,int WINDOW_HEIGHT)
{

	XMMATRIX mat = XMMatrixScaling((float)WIDTH / WINDOW_WIDTH, (float)HEIGHT / WINDOW_HEIGHT, 1.0f);

	CONSTANT_BUFFER cb;
	cb.matW = XMMatrixTranspose(mat* worldMatrix);
...
//各バッファをパイプラインにセット
void Spirete::SetBufferToPipeline()
{
	//頂点バッファ
